...
// Чистим pending debounce при unmount — см. PointsAwardModal.vue.
onUnmounted(() => {
  if (searchTimeout) {
    clearTimeout(searchTimeout)
    searchTimeout = null
...
async function handleSubmit() {
  ...
  const success = await creditsService.award({
  ...
  >
    <DialogContent class="sm:max-w-[450px]">
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            type="number"
            placeholder="Может быть отрицательной (списание)"
            class="mt-1"
          />
        </div>
      ...
      <DialogFooter>
        ...
        <Button
          :disabled="!selectedMemberId || amount === 0 || creditsService.isLoading.value"
          @click="handleSubmit"
        >
          Начислить
        </Button>
