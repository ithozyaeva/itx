...
func main() {
	...
	if config.CFG.AppMode != "bot" {
		...
		app := fiber.New(fiber.Config{
			AppName: "ITX API",
			// 12 MB: верхняя граница для multipart-аплоадов (фото маркетплейса
			// и резюме лимитированы в 10 MB; +2 MB на накладные расходы формы).
			// Без BodyLimit fiber по умолчанию принимает только 4 MB, но и
			// бесконечный приём опасен — даём явный предел.
