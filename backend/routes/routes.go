...
func SetupAdminRoutes(app *fiber.App, db *gorm.DB, redisClient *redis.Client) {
	...
	if redisClient != nil {
		...
		subs.Post("/chats", authMiddleware.RequirePermission(models.PermissionCanEditAdminSubscriptions), subscriptionHandler.CreateChat)
		subs.Put("/chats/:id", authMiddleware.RequirePermission(models.PermissionCanEditAdminSubscriptions), subscriptionHandler.UpdateChat)
		subs.Delete("/chats/:id", authMiddleware.RequirePermission(models.PermissionCanEditAdminSubscriptions), subscriptionHandler.DeleteChat)
