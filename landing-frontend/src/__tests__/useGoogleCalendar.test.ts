...
describe('useGoogleCalendar', () => {
  describe('formatDateForGoogle', () => {
    ...
    it('handles end-of-day time', () => {
      ...
      expect(formatDateForGoogle(date)).toBe('20240615T235959Z')
    })
  })
  ...
  describe('buildGoogleCalendarUrl', () => {
    ...
    it('uses endDate when present', () => {
      const { buildGoogleCalendarUrl } = useGoogleCalendar()
      const event = createEvent({ endDate: '2024-01-15T14:00:00.000Z' } as any)
      const url = new URL(buildGoogleCalendarUrl(event))

      expect(url.searchParams.get('dates')).toBe('20240115T103000Z/20240115T140000Z')
    })
