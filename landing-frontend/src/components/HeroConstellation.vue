...
interface Node {
  ...
  hub: boolean
  pulse: number
  // Live position in normalized space (-1..1)
  posX: number
  posY: number
}
...
function buildNodes(): Node[] {
  ...
  for (const ring of RINGS) {
    const count = Math.round(NODE_COUNT * ring / RINGS.reduce((a, b) => a + b))
...
function readPalette(): Palette {
  ...
  const accent = typeof window === 'undefined'
    ? fallback
    : (getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || fallback)
  return {
    raw: accent,
    fg: `hsl(${accent})`,
    hub: '#ffb547',
...
onMounted(() => {
  ...
  const cx = cssSize / 2
  const cy = cssSize / 2
  const R = cssSize * 0.48 // logical radius for the canvas drawing
  ...
  let lastPulse = -2
  let pulseT = -1 // -1 = idle, otherwise progress 0..1
  ...
  function compute(t: number) {
    ...
    // Animated positions in normalized (-1..1) space
  ...
  function drawEdges() {
    // k-NN graph (k=2) + a few hub-to-hub long links
    const k = 2
